...
var createPoParseFn = function(lang) {
  return function(msgDir) {
    var fp = path.join(msgDir, lang, 'LC_MESSAGES', 'messages.po');
    var po = '';
    try{
      po = fs.readFileSync(fp, {encoding: 'utf8'});
    }catch(err) {
      if(err.code !== 'ENOENT') {
        throw err;
      }
...
var createMsgParseFn = function(defaultLang, msgDirsArray) {
  return function(lang) {
    if (lang === defaultLang) {
      return {};
    }
    var parsePoFn = createPoParseFn(lang);
    var jsonPoArray = lodash.map(msgDirsArray, parsePoFn);
    return lodash.reduce(jsonPoArray, lodash.assign);
  };
};
...
Translator.prototype.getCatalogs = function() {
  ...
  var parseMessages = createMsgParseFn(this.options.defaultLang, this.options.msgDirs);
